...
def main():
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                lora_transformer_state_dict = get_peft_model_state_dict(transformer_, adapter_name="default")
                lora_text_encoder_state_dict = get_peft_model_state_dict(text_encoder_, adapter_name="default")
                StableDiffusionPipeline.save_lora_weights(os.path.join(output_dir, "trans_lora"),
                                                          lora_transformer_state_dict)
                StableDiffusionPipeline.save_lora_weights(os.path.join(output_dir, "text_lora"),
                                                          lora_text_encoder_state_dict)
                # save weights in peft format to be able to load them back
                transformer_.save_pretrained(os.path.join(output_dir, "transformer_lora"))
        ...
        def load_model_hook(models, input_dir):
            ...
            transformer_ = accelerator.unwrap_model(transformer)
            transformer_.load_adapter(os.path.join(input_dir, "transformer_lora"), "default", is_trainable=True)
            text_encoder_ = accelerator.unwrap_model(text_encoder)
            text_encoder_.load_adapter(os.path.join(input_dir, "text_encoder_lora"), "default", is_trainable=True)
    ...
    lora_layers = list(filter(lambda p: p.requires_grad, transformer.parameters()))+ list(filter(lambda p: p.requires_grad, text_encoder.parameters()))
    ...
    def collate_fn(examples):
        ...
        return {"pixel_values": pixel_values, "text":text,"input_ids": input_ids, 'prompt_attention_mask': prompt_attention_mask}
    ...
    transformer,text_encoder, optimizer, train_dataloader, lr_scheduler = \
        accelerator.prepare(transformer,text_encoder, optimizer, train_dataloader, lr_scheduler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                noisy_latents = noise_scheduler.step(model_pred, timesteps, noisy_latents, return_dict=False)[0].to(weight_dtype)
                ...
                if global_step % args.N_steps  == 0:
                    ...
                    prompt_embeds, prompt_attention_mask, negative_embeds, negative_prompt_attention_mask = pipe_textencoder.encode_prompt(
                            batch["text"])
                    ...
                    print(f"edit_latents dtype: {edit_latents.dtype}")
                    image = vae.decode(edit_latents.to(dtype=weight_dtype) / vae.config.scaling_factor, return_dict=False)[0]
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        unwrapped_text_encoder = accelerator.unwrap_model(text_encoder, keep_fp32_wrapper=False)
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                pipeline = DiffusionPipeline.from_pretrained(
                    ...
                    text_encoder=accelerator.unwrap_model(text_encoder, keep_fp32_wrapper=False), vae=vae,
                    torch_dtype=weight_dtype,
                )
    ...
    if accelerator.is_main_process:
        ...
        StableDiffusionPipeline.save_lora_weights(os.path.join(args.output_dir, "text_lora"),
    ...
    # load lora weight
    transformer = PeftModel.from_pretrained(transformer, os.path.join(args.output_dir,"transformer_lora"))
    text_encoder = T5EncoderModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision, torch_dtype=weight_dtype,
    )
    text_encoder = PeftModel.from_pretrained(text_encoder, os.path.join(args.output_dir,"text_encoder_lora"))
